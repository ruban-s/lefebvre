"use client";
import {
  IndirectReportSchema,
  LabourTicketReportSchema,
...
interface LabourTicketReportFormContainerProps {
  defaultValue: {
    from_date: Date;
    to_date: Date;
  };
...
const LabourTicketReportFormContainer = ({
  defaultValue,
  changeFilterValue,
}: LabourTicketReportFormContainerProps) => {
  const [dateRange, setDateRange] = useState<DateRange | undefined>();
  const [disabledDates, setDisbleDates] = useState<Date[]>([]);
  const [fromDate, setFromDate] = useState<string>("");
  const [toDate, setToDate] = useState<string>("");
  const [disable, setDisable] = useState<boolean>(true);
  const form = useForm<z.infer<typeof LabourTicketReportSchema>>({
    resolver: zodResolver(LabourTicketReportSchema),
    defaultValues: {
      start_date: "",
      end_date: "",
    },
...
export default LabourTicketReportFormContainer;
