...
import {
  ...
  WorkOrderReportSchema,
} from "@/schemas/reportindex";
...
interface ResourceReportFormContainerProps {
...
const ResourceReportFormContainer = ({
  changeFilterData,
  defaultData,
  workOrderList,
  fetchWorkOrder,
}: ResourceReportFormContainerProps) => {
  const [selectedProject, setProject] = useState<ProjectData | undefined>();
  const [disable, setDisable] = useState<boolean>(true);
  const form = useForm<z.infer<typeof ResourceReportSchema>>({
    resolver: zodResolver(ResourceReportSchema),
    defaultValues: {
      status: "",
      project_id: "",
      work_order_Id: "",
  ...
  const onSubmit = (value: z.infer<typeof ResourceReportSchema>) => {
    changeFilterData(value.status, value.project_id, value.work_order_Id);
  };
...
export default ResourceReportFormContainer;
